...
class ResponseFormatter:
    ...
    def format_json(self, data: List[Dict[str, Any]], pretty: bool = True) -> str:
        """
        ...
        serializable_data = [
            {k: self._make_serializable(v) for k, v in row.items()} for row in data
        ]
    ...
    def format_summary(
        self, data: List[Dict[str, Any]], key_metrics: Optional[List[str]] = None
    ) -> str:
        """
        ...
        lines = [f"Results Summary", f"Total Rows: {len(data)}", ""]
        ...
        if len(data) == 1:
            ...
            if key_metrics:
                ...
                for metric in key_metrics:
                    ...
                    numeric_values = [
                        v for v in values if isinstance(v, (int, float, Decimal))
                    ]

                    if numeric_values:
                        lines.append(f"  {metric}:")
                        lines.append(
                            f"    Total: {self._format_value(sum(numeric_values))}"
                        )
                        lines.append(
                            f"    Average: {self._format_value(sum(numeric_values) / len(numeric_values))}"
                        )
                        lines.append(
                            f"    Min: {self._format_value(min(numeric_values))}"
                        )
                        lines.append(
                            f"    Max: {self._format_value(max(numeric_values))}"
                        )
                lines.append("")
...
""")
        ...
        table = tabulate(
            [
                [self._format_value(row.get(col)) for col in data[0].keys()]
                for row in data
            ],
...
class AnalysisResponse:
    ...
    def to_markdown(self) -> str:
        ...
        if self.narrative:
            lines.extend([f"## Summary", f"", self.narrative, f""])
    ...
    def to_html(self) -> str:
        ...
        if self.narrative:
            html_parts.extend(["<h2>Summary</h2>", f"<p>{self.narrative}</p>"])
