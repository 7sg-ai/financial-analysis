...
class QueryGenerator:
    ...
    def __init__(
        ...
        self.client = AzureOpenAI(
            azure_endpoint=normalized_endpoint, api_key=api_key, api_version=api_version
        )
    ...
    def generate_query(
        ...
        try:
            ...
            logger.info(
                f"Successfully generated query: {result.get('query', '')[:100]}..."
            )
            return result
            ...
            logger.error(
                f"Response content: {result_text_safe[:500] if result_text_safe != 'N/A' else 'N/A'}..."
            )
            raise ValueError(
                f"Model did not return valid JSON. Response: {result_text_safe[:200] if result_text_safe != 'N/A' else 'N/A'}..."
            )
    ...
    def validate_query(self, query: str) -> Dict[str, Any]:
        ...
        if "SELECT *" in query_upper and "LIMIT" not in query_upper:
            warnings.append(
                "Query uses SELECT * without LIMIT - may return large results"
            )
    ...
    def refine_query(
        self, original_question: str, original_query: str, user_feedback: str
    ) -> Dict[str, Any]:
        """
...
"""
    ...
    def suggest_related_queries(
        self, user_question: str, num_suggestions: int = 3
    ) -> List[str]:
        """
...
"""
    ...
    def _build_user_prompt(self, user_question: str, include_explanation: bool) -> str:
...
class NarrativeGenerator:
    ...
    def generate_narrative(
        ...
        results_preview = (
            query_results[:20] if len(query_results) > 20 else query_results
        )
