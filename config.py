...
class Settings(BaseSettings):
    ...
    azure_openai_deployment_name: str = Field(
        default="gpt-5.2-chat", env="AZURE_OPENAI_DEPLOYMENT_NAME"
    )
    ...
    azure_storage_account_name: Optional[str] = Field(
        default=None, env="AZURE_STORAGE_ACCOUNT_NAME"
    )
    azure_storage_account_key: Optional[str] = Field(
        default=None, env="AZURE_STORAGE_ACCOUNT_KEY"
    )
    azure_storage_container: Optional[str] = Field(
        default=None, env="AZURE_STORAGE_CONTAINER"
    )
