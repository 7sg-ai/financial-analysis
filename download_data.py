...
def download_file(url: str, filepath: Path, retries: int = 3) -> bool:
    ...
    for attempt in range(retries):
        try:
            logger.info(
                f"Downloading {filepath.name}... (attempt {attempt + 1}/{retries})"
            )
            ...
            if response.status_code == 403:
                logger.warning(
                    f"⚠️  Access forbidden (403) for {filepath.name} - data may not be publicly available"
                )
                return False
            ...
            if response.status_code == 404:
                logger.warning(
                    f"⚠️  File not found (404) for {filepath.name} - may not exist for this date"
                )
                return False
            ...
            if e.response.status_code in [403, 404]:
                ...
                logger.warning(
                    f"⚠️  Skipping {filepath.name}: HTTP {e.response.status_code}"
                )
                return False
            ...
            if attempt < retries - 1:
                ...
                logger.error(
                    f"❌ Failed to download {filepath.name} after {retries} attempts"
                )
...
def download_parquet_data(
    service_types=None, years=None, months=None, skip_existing=True
):
...
def get_storage_config() -> Optional[Tuple[str, str, str]]:
    ...
    """
    account_name = os.environ.get("AZURE_STORAGE_ACCOUNT_NAME") or os.environ.get(
        "SYNAPSE_STORAGE_ACCOUNT"
    )
    account_key = os.environ.get("AZURE_STORAGE_ACCOUNT_KEY")
    container = os.environ.get("AZURE_STORAGE_CONTAINER") or os.environ.get(
        "SYNAPSE_FILE_SYSTEM", "data"
    )
...
def upload_file_to_storage(
    ...
    try:
        ...
        logger.info(
            f"☁️  Uploaded {local_path.name} -> {blob_path} ({file_size_mb:.1f} MB)"
        )
        return True
    except ImportError:
        logger.error(
            "azure-storage-blob package required for upload. Install with: pip install azure-storage-blob"
        )
        return False
    except Exception as e:
...
def upload_to_storage(
    ...
    if zones_file.exists():
        blob_path = (
            f"{prefix}/taxi_zone_lookup.csv" if prefix else "taxi_zone_lookup.csv"
        )
        total_count += 1
        if upload_file_to_storage(
            zones_file,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download NYC Taxi Data (2023-2025) - Azure Deployment"
    )
    parser.add_argument(
        "--years",
        nargs="+",
    ...
    if args.upload_only:
        logger.info(
            "🚕 Upload-only mode - uploading existing src_data to Azure Storage"
        )
        logger.info("=" * 70)
        data_dir = Path("src_data")
        if not data_dir.exists():
            logger.error(
                "src_data directory not found. Run from project root or where src_data exists."
            )
            exit(1)
        storage_config = get_storage_config()
        if not storage_config:
            logger.error(
                "Storage config required. Set AZURE_STORAGE_ACCOUNT_NAME, AZURE_STORAGE_ACCOUNT_KEY"
            )
            exit(1)
    ...
    logger.info(
        f"📈 Total Files: {len(years) * len(months) * len(args.service_types)} parquet files"
    )
    ...
    if args.upload:
        ...
        if storage_config:
            ...
            logger.warning(
                "⚠️  Upload skipped: Set AZURE_STORAGE_ACCOUNT_NAME, AZURE_STORAGE_ACCOUNT_KEY (and optionally AZURE_STORAGE_CONTAINER)"
            )
    ...
    if success == total and zones_success:
        ...
        if args.upload and upload_success and upload_success > 0:
            logger.info(
                "☁️  Data uploaded to Azure Storage - Spark pool can now access it"
            )
            logger.info(
                "   Set DATA_PATH in your API environment to the path shown above"
            )
        else:
