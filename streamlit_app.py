...
if "api_url" not in st.session_state or st.session_state.api_url != env_api_url:
    ...
    if "api_url" in st.session_state and st.session_state.api_url != env_api_url:
        logger.info(
            f"API URL changed from {st.session_state.api_url} to {env_api_url}, resetting connection"
        )
...
def initialize_api():
    ...
    if st.session_state.api_url != env_api_url:
        logger.info(
            f"API URL changed, updating from {st.session_state.api_url} to {env_api_url}"
        )
        st.session_state.api_url = env_api_url
        st.session_state.api_available = False
    ...
    if not st.session_state.api_available:
        ...
        with st.spinner("Connecting to Financial Analysis API..."):
            if check_api_health():
                ...
                st.error(
                    f"❌ Failed to connect to API backend at {st.session_state.api_url}"
                )
...
def call_api(
    question: str, return_format: str = "both", include_narrative: bool = True
    ...
    logger.debug(
        f"Parameters: return_format={return_format}, include_narrative={include_narrative}"
    )
    ...
    try:
        ...
        logger.info(
            f"API response received. Response keys: {list(response_json.keys())}"
        )
        logger.debug(
            f"Response result_count: {response_json.get('result_count', 'N/A')}"
        )
        logger.debug(f"Response results type: {type(response_json.get('results'))}")
        logger.debug(
            f"Response results length: {len(response_json.get('results', []))}"
        )
        ...
        if response_json.get("results"):
            logger.debug(
                f"First result sample: {response_json['results'][0] if len(response_json['results']) > 0 else 'N/A'}"
            )
        else:
        ...
        logger.error(
            f"Response text: {e.response.text if hasattr(e, 'response') and hasattr(e.response, 'text') else 'N/A'}"
        )
...
def display_results(results: Dict[str, Any]):
    ...
    logger.debug(
        f"Results.get('results'): {results.get('results') if results else 'N/A'}"
    )
    logger.debug(
        f"Results.get('results') type: {type(results.get('results')) if results else 'N/A'}"
    )
    logger.debug(
        f"Results.get('results') length: {len(results.get('results', [])) if results else 'N/A'}"
    )
    logger.debug(
        f"Results.get('result_count'): {results.get('result_count') if results else 'N/A'}"
    )
    ...
    if not results.get("results"):
        logger.warning(
            f"display_results: results.get('results') is empty. Full results dict: {results}"
        )
        validation_errors = results.get("metadata", {}).get("validation_errors", [])
        is_security_rejection = any(
            "dangerous keyword" in str(e).lower()
            or "forbidden operation" in str(e).lower()
            for e in validation_errors
        )
        if is_security_rejection:
            st.error(
                "Query was stopped due to security concerns with the SQL statement"
            )
        else:
            st.warning("No results to display")
    ...
    with col3:
        st.metric(
            "Query Type", results.get("metadata", {}).get("query_type", "Unknown")
        )
    with col4:
        st.metric(
            "Tables Used", len(results.get("metadata", {}).get("tables_used", []))
        )
...
def main():
    ...
    # Header
    st.markdown(
        '<h1 class="main-header">🚕 Financial Analysis Dashboard</h1>',
        unsafe_allow_html=True,
    )
    st.markdown(
        "**Analyze NYC Taxi & For-Hire Vehicle Data with Natural Language Queries**"
    )
    ...
    with st.sidebar:
        ...
        if st.session_state.api_available:
            ...
            if env_api_url != st.session_state.api_url:
                st.warning(
                    f"⚠️ Env API_URL ({env_api_url}) differs from session ({st.session_state.api_url})"
                )
        else:
        ...
        if st.session_state.query_history:
            ...
            for i, (question, timestamp) in enumerate(
        ...
        if st.session_state.api_available:
            ...
            try:
                ...
                r = httpx.get(
                    f"{st.session_state.api_url}/api/data-status", timeout=10.0
                )
                if r.status_code == 200:
                    data = r.json()
                    for view_name, detail in (
                        data.get("views", {}).get("details", {}).items()
                    ):
    ...
    if st.session_state.query_history:
        ...
        for i, (q, timestamp) in enumerate(st.session_state.query_history[-3:]):
            with st.expander(
                f"Query {len(st.session_state.query_history) - i}: {q[:50]}..."
            ):
                st.text(f"Time: {timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
                st.text(f"Question: {q}")
                if st.button(
                    f"Re-run Query {len(st.session_state.query_history) - i}",
                    key=f"rerun_{i}",
                ):
