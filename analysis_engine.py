...
class FinancialAnalysisEngine:
    ...
    def initialize_data(
        self, months: Optional[List[str]] = None, year: Optional[str] = None
    ) -> None:
        """
        ...
        if success:
            ...
            logger.warning(
                "No data files found at startup. Application will start but queries may fail until data is available."
            )
    ...
    def analyze(
        ...
        try:
            ...
            query_result = self.query_generator.generate_query(
                user_question=question, include_explanation=True, max_rows=max_rows
            )
            ...
            if not validation["is_valid"]:
                error_msg = (
                    f"Query validation failed: {', '.join(validation['issues'])}"
                )
            ...
            logger.debug(
                f"Query execution result keys: {list(execution_result.keys())}"
            )
            logger.debug(f"Query execution success: {execution_result.get('success')}")
            logger.debug(
                f"Query execution error: {execution_result.get('error', 'None')}"
            )
            logger.debug(
                f"Query execution data type: {type(execution_result.get('data'))}"
            )
            logger.debug(
                f"Query execution data length: {len(execution_result.get('data', []))}"
            )
            ...
            logger.info(
                f"Query executed successfully: {len(results)} rows in {execution_time}ms"
            )
            ...
            if results:
                ...
                logger.debug(
                    f"First result sample: {results[0] if len(results) > 0 else 'N/A'}"
                )
            else:
            ...
            if include_narrative and return_format in ["narrative", "both"]:
                ...
                try:
                    narrative = self.narrative_generator.generate_narrative(
                        user_question=question, query_results=results, query_used=query
                    )
            ...
            logger.debug(
                f"Results before creating response: {results[:2] if len(results) > 0 else 'Empty'}"
            )
            ...
            logger.debug(
                f"Response created. Response.results type: {type(response.results)}"
            )
